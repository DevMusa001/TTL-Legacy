...
fn test_add_vesting_milestone_succeeds_for_owner() {
    ...
    client.add_vesting_milestone(
        &vault_id,
        &"Beneficiary turns 18".to_string(),
        &attestor,
    );
...
fn test_release_blocked_with_multiple_unattested_milestones() {
    ...
    client.add_vesting_milestone(
        &vault_id,
        &"Completes education".to_string(),
        &attestor2,
    );
    let attestor3 = Address::generate(&env);
    client.add_vesting_milestone(
        &vault_id,
        &"Employed for 1 year".to_string(),
        &attestor3,
    );
...
fn test_add_milestone_with_descriptive_name() {
    ...
    let description = "Beneficiary completes university education and obtains bachelor's degree".to_string();
